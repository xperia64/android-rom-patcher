...
void fputb(byte x)  { inc_pc(1); if(!pass)return; fputc(x, wr); }
...
void reset()
{
    ...
    macroline =new string();
    lnamespace=new string();
	pass = filenum = linenum = blocknum = opnum = opargs = oparg_labels = 
  oparg_labels_unresolved=0; //0=all labels resolved, 1=label used before it was defined (in pass 0)
	state.pc=0;           //rom position
  state.mode=0;         //lorom or hirom
  state.header=0;       //whether or not to skip 512 byte header
  state.fillbyte=0;     //byte to write with 'fill' command
  state.padbyte=0;      //byte to write with 'pad' command
  state.base=0;         //used for relocatable code
  state.basepc=0;       //used for relocatable code
  state.table[256];   //table for db strings
  for(loop=0; loop<256; loop++)
	  state.table[loop]=0;
  state.rep=0;          //repeat counter for 'rep' command
  state.bytecount=0;    //counter for bytes written
  state.opcount=0;      //counter for opcodes written
  state.inmacro=0;      //macro recursion counter
  state.macronum=0;     //macro call counter used for macro labels
 state.retmacro=0;     //true only after first return from a macro
  state.retfile=0;      //return file flag
  state.brcount[6];   //branch counters [+,++,+++,-,--,---]
 
  for(loop=0; loop<6; loop++)
	  state.brcount[loop]=0;

assume.f_mx=assume.f_db=assume.f_d=0;
  assume.mx=0;
  assume.db=0;
  assume.d=0;
  ...
  {
	  file_list[loop].data=NULL;
	  file_list[loop].fn=NULL;
	  file_list[loop].fsize=0;
	  file_list[loop].pfilenum=file_list[loop].plinenum=file_list[loop].pblocknum=0;
  }
file_count=0;
}
